var ProxyServer = require('http-proxy').createProxyServer
...
function create (options) {
  ...
  if (options.server) {
    server = new URL(options.server.replace(options.urlPath, ''))
...
function wsProxy (options) {
  var server = new URL(options.server.replace(options.urlPath, '').replace(/^http/, 'ws'))
  var proxy = new ProxyServer({
    target: server,
    ws: true,
    secure: false
