function lock () {
  var counter = 1; var cb
  var L = function (f) {
    counter++
    return function () {
      var retval = f.apply(this, arguments)
      if (!--counter) cb()
      return retval
    }
  }
  L.done = function (callback) {
    cb = callback
    if (!--counter) cb()
  }
  return L
}
function create (options) {
  var fs = require('fs')
  var http = require('http')
  var https = require('https')
  var path = require('path')

  var verbose = options.verbose
  var manifests = options.manifests.filter(function (dir) {
    var include = false

    try {
      include = fs.statSync(dir).isDirectory()
    } catch (e) {}
    if (!include && verbose.local) {
      console.log('Ignoring', dir, ' as manifest directory.')
    }
    return include
  })

  var urlPath = options.urlPath

  var loginPath = '/api/login'

  var server
  var protocol

  if (options.server) {
    server = new URL(options.server)
    if (server.protocol !== 'http:' && server.protocol !== 'https:') {
      console.error('Server must be an HTTP(S) URL')
      return
    }
    protocol = server.protocol === 'https:' ? https : http
    loginPath = urlPath + loginPath.slice(1)
  }

  return function proxyLogin (request, response, next) {
    console.log()
    var url = new URL(request.url, options.server)
    if (!(((request.method === 'GET') &&
      (url.pathname === loginPath) &&
      (url.searchParams.get('rampup') === 'true')) ||
            ((request.method === 'POST') &&
      (url.pathname === loginPath)))) {
      return next()
    }

    if (!options.server) {
      console.error('Login proxy requires --server')
      response.writeHead(501, 'Login proxy requires --server',
        { 'Content-Type': 'text/plain' })
      response.end('Login proxy requires --server')
      return next()
    }
    var remoteUrl = new URL(request.url.slice(urlPath.length), options.server)
    var opt = remoteUrl
    opt.headers = request.headers
    opt.headers.host = opt.host
    opt.method = request.method
    delete opt.headers['accept-encoding']
    opt.rejectUnauthorized = options.rejectUnauthorized
    var req = protocol.request(opt, function (res) {
      if (res.statusCode !== 200) {
        response.writeHead(res.statusCode, res.headers)
        res.pipe(response)
        return
      }
      var headers = res.headers
      var cookies = headers['set-cookie']
      if (cookies) {
        if (typeof cookies === 'string') cookies = [cookies]
        headers['set-cookie'] = cookies.map(function (s) {
          return s.replace(/\sdomain=[^;]*;?/ig, '').replace(/\ssecure;?/ig, '')
        })
      }
      response.writeHead(res.statusCode, headers)
      if (request.method === 'POST') {
        res.pipe(response)
        return
      }
      var reply = []; var map = {}; var L = lock()
      res.on('data', data).on('end', end)
      function data (chunk) { reply.push(chunk) }
      function end () {
        reply = JSON.parse(reply.join(''))
        if (reply.error || !reply.rampup) {
          if (verbose.local) console.log('Invalid/no rampup data received from backend.')
          response.end(JSON.stringify(reply, null, 4))
          return
        }
        var list = reply.rampup.serverConfig.manifests
        for (var i in list) map[list[i].path] = list[i]
        manifests.forEach(readDir)
        L.done(sendReply)
      }
      function readDir (dir) {
        fs.readdir(dir, L(function (err, files) {
          if (err) return console.error(err.message)
          files = files.filter(str => /\.json$/i.test(str))
          files.forEach(function (file) {
            file = path.join(dir, file)
            if (verbose.local) console.log(file)
            fs.readFile(file, 'utf8', L(addManifest))
          })
        }))
      }
      function addManifest (err, manifest) {
        if (err) return console.error(err.message)
        manifest = JSON.parse(manifest)
        if (!(manifest instanceof Array)) manifest = [manifest]
        for (var i in manifest) map[manifest[i].path] = manifest[i]
      }
      function sendReply () {
        var list = reply.rampup.serverConfig.manifests = []
        for (var i in map) {
          list.push(map[i])
        }
        response.end(JSON.stringify(reply, null, 4))
        if (verbose.local) console.log()
      }
    })
    if (request.method === 'POST') {
      request.pipe(req)
    } else {
      req.end()
    }
  }
}

module.exports = {
  create: create
}
