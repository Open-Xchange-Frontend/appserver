...
function createRemoteReadStream (request, urlPath) {
  return new Promise(function (resolve, reject) {
    const options = request.app.get('options')
    // eslint-disable-next-line
    const server = url.parse(options.server)
    // eslint-disable-next-line
    const URL = url.resolve(server, urlPath)
    const protocol = protocolOf(options.server)
...
function writeToStream ([fileName, readStream]) {
  return new Promise(function (resolve, reject) {
...
function fileExistsInBasepath (options, fileName) {
  return new Promise(function (resolve, reject) {
    // eslint-disable-next-line
    if (!fileName) reject({})
    const filePath = path.resolve(options.prefixes[0], fileName)
    fs.access(
      filePath,
...
function needsUpdate (options, fileName) {
  ...
  return function (err) {
    ...
    const mtime = fs.statSync(path.resolve(options.prefixes[0], fileName)).mtime
    return Promise.all(
      options.prefixes.slice(1)
        .map(pathFinder)
    ).catch(function (file) {
...
function staticFiles (request, response, next) {
  const moduleName = request.params[0] || 'ui'
  const options = request.app.get('options')
  let version = ''
