...
function createRemoteReadStream (request, urlPath) {
  return new Promise(function (resolve, reject) {
    ...
    var url = new URL(urlPath, options.server)
    var protocol = protocolOf(options.server)
...
function readFromStream (options, request, fileName, urlPath) {
  var pathFinder = createPathFinder(
    ...
    fileName.replace(path.resolve(options.prefixes[0]), '')
  )
  return Promise.all([
      ...
      .then(
        () =>
          createRemoteReadStream(request, urlPath)
            .catch((err) => {
              if (options.verbose.remote) console.error(urlPath, err)
              throw err
            }),
        function (localFile) {
...
function fileExistsInBasepath (options, fileName) {
  return new Promise(function (resolve, reject) {
    if (!fileName) reject(new Error())
    var filePath = path.resolve(options.prefixes[0], fileName)
    fs.access(
      filePath,
      fs.constants.F_OK | fs.constants.W_OK,
      (err) => err ? resolve(err) : reject(new Error('fileExists'))
    )
  })
}
...
function needsUpdate (options, fileName) {
  ...
  return function (err) {
    if (err.message !== 'fileExists') throw err

    var mtime = fs.statSync(path.resolve(options.prefixes[0], fileName)).mtime
    return Promise.all(
      options.prefixes.slice(1)
        .map(pathFinder)
    ).catch(function (file) {
      if (mtime < fs.statSync(file).mtime) fs.unlinkSync(path.resolve(options.prefixes[0], fileName))
...
function appsLoadFile (options, request, fileName, urlPath) {
  return fileExistsInBasepath(options, 'apps/' + fileName)
    ...
    .then(function (err) {
      if (err.code === 'ENOENT') { return err.path }
      throw err
    })
...
function appsLoad (request, response, next) {
  var url = new URL(request.url, request.app.get('options').server)
  var options = request.app.get('options')
  ...
  Promise.all(list.map(function (moduleName) {
    ...
    var urlPath = urlPathFor(moduleName, version)
...
function staticFiles (request, response, next) {
  ...
  if (request.params.version) version = 'v=' + request.params.version + path.sep
  if (/^v=[^/]*\//.test(moduleName) || /^api\//.test(moduleName)) return next()
  return fileExistsInBasepath(options, moduleName)
    .catch(needsUpdate(options, moduleName))
    .then(function (err) {
      if (err.code === 'ENOENT') { return err.path || path.resolve(options.prefixes[0], moduleName) }
      throw err
    })
    .then((fileName) => readFromStream(options, request, fileName, fileName.replace(path.resolve(options.prefixes[0]) + path.sep, version)))
    .then(writeToStream)
    .catch((err) => options.verbose['local:error'] && err.fileExists !== true ? console.error(err) : '')
    .then(() => next())
}
...
function fetchManifests (options) {
  return fileExistsInBasepath(options, 'manifests/server.json')
    .then(function (err) {
      if (err.code === 'ENOENT') { return err.path || path.resolve(path.join(options.prefixes[0], 'manifests/server.json')) }
      throw err
    })
    .then(function (fileName) {
      return new Promise(function (resolve, reject) {
        protocolOf(options.server).get(
          new URL('api/apps/manifests?action=config', options.server),
          function (res) {
            if (res.statusCode !== 200) return reject(res.statusMessage)
    ...
    .then(function ([fileName, res]) {
      ...
      return new Promise(function (resolve, reject) {
        res.on('end', function () {
          ...
          const manifests = JSON.parse(buf).data.manifests
...
function create (options) {
  ...
  // index (=> /ui) + static files without version
