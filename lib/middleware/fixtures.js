...
function create(app, fixturesPath) {
    const getFixture = createFixtures(fixturesPath || process.env.fixturespath);
    ...
    app.all('*', function (req, res, next) {
        ...
        if (req.url.indexOf('/appsuite') >= 0) {
            [prefix, postfix] = req.url.split('/appsuite');
            url = `/appsuite${postfix ? postfix : ''}`;
...
module.exports = {
    create: (options) => create(express(), options.path)
};
