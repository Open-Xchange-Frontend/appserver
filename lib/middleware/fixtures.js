...
const createFixtures = (function () {
  ...
  return function (dir) {
    const fixtures = scanDir(dir).then(list => {
      ...
      console.error('Could not find any fixtures. Try to configure the path to fixtures correctly.')
      return {}
    })
...
module.exports = function (options) {
  const app = express()
  const fixturesPath = path.isAbsolute(options.fixturesPath) ? options.fixturesPath : path.join(options.prefixes[0], options.fixturesPath)
